def validation_chema(df: pd.DataFrame, model_class: ModelProtocol, start: int = 0, window_size: int = -1)->pd.DataFrame:
    ...
    for month in range(start, max_month):
        ...
        valid = df[df["date_block_num"] == month + 1][['corrected_shop_id', 'corrected_item_id', 'main_category',  'item_cnt_day']]
        valid = (
            valid
        ...
        results = pd.concat([results, valid[['date_block_num', 'corrected_shop_id', 'corrected_item_id', 'item_cnt_day', 'true_ans']]], ignore_index=True)
    return results
